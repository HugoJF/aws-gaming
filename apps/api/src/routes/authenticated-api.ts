...
export function createAuthenticatedApiRoutes(
  deps: Pick<AppDeps, 'repo' | 'statusService' | 'costService' | 'authMiddleware' | 'adminMiddleware'>,
) {
  const api = new Hono();
  ...
  api.get('/servers', async (c) => {
    ...
    const instances = await Promise.all(
      gameInstanceIds.map((id) => deps.repo.getInstance(id)),
    );

    const views = await Promise.all(
      instances
        .filter((inst): inst is NonNullable<typeof inst> => inst !== null)
        .map((inst) => deps.statusService.buildServerView(inst)),
    );
  ...
  api.get('/servers/:id/status', async (c) => {
    ...
    const instance = await deps.repo.getInstance(id);
    if (!instance) {
      return c.json({ error: 'Server not found' }, 404);
    }

    const view = await deps.statusService.buildServerView(instance);
  ...
  api.get('/servers/:id/cost', async (c) => {
    ...
    try {
      const estimate = await deps.costService.estimateHourlyCosts(instance);
  ...
  api.post('/servers/:id/power', async (c) => {
    ...
    try {
      current = await deps.statusService.buildServerView(instance);
    } catch (error) {
    ...
    try {
      await deps.statusService.startTransition(instance, body.action);
    } catch (error) {
    ...
    let view: ServerView;
    try {
      view = await deps.statusService.buildServerView(instance);
    } catch (error) {
  ...
  api.route('/admin', createAdminRoutes(deps));
